...
int main(int argc, char **argv)
{
    ...
    //buffer_image
     ...
     int buf_size = 4 * my_image_width * my_image_height;
    char * buffer_image = new char[buf_size];
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (!BasicTracer::rendering)
        {
            auto worker = std::thread(BasicTracer::generate_frame);
            worker.detach();
 
            //generate_frame();
            BasicTracer::rendering = true;
        }
        else
        if (BasicTracer::frame_ready)
        {
            
            for (int i = 0; i < my_image_width * my_image_height; i++)
            {
                buffer_image[i * 4 + 0] = std::fmin(float(1), BasicTracer::image[i].x) * 255;
                buffer_image[i * 4 + 1] = std::fmin(float(1), BasicTracer::image[i].y) * 255;
                buffer_image[i * 4 + 2] = std::fmin(float(1), BasicTracer::image[i].z) * 255;
                buffer_image[i * 4 + 3] = 255;
            }
                
            delete [] BasicTracer::image;

            BasicTracer::rendering = false;
            BasicTracer::frame_ready = false;
