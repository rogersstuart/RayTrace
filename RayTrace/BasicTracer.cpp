...
volatile int BasicTracer::num_threads = 0;
Vec3f* BasicTracer::image;
void BasicTracer::render(const std::vector<Sphere>& spheres)
{
        ...
        auto thread = std::thread([invWidth, angle, aspectratio, invHeight, spheres, y, width, pixel]
            {
                for (unsigned x = 0; x < width; ++x)
                {
                    ...
                    raydir.normalize();
                    pixel[y * width + x] = BasicTracer::trace(Vec3f(0), raydir, spheres, 0);
                }
...
bool BasicTracer::frame_ready = false, BasicTracer::rendering = false;
Vec3f* BasicTracer::image_buffer;

float BasicTracer::sph_1 = 0;
void BasicTracer::generate_frame()
{
    std::cout << "here\n";
...
float BasicTracer::mix(const float& a, const float& b, const float& mix)
{
